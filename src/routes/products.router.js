...
//Params Id
router.get('/id/:pid', (request, response) => {
	try {
		if (fs.existsSync(fileName)) {
			let products = fs.readFileSync(fileName, "utf-8")
...
//Para agregar un producto. Funciona con Moongose :)
router.post('/', uploader.array('files'), async (request, response) => {
	try {
		if (fs.existsSync(fileName)) {
			...
			//monogo
			const newProduct = await productsModel.create({ id: idNewProduct, title, description, code, price, stock, category: categoryLC, thumbnail: thumbnailImages, status })

			// Agregamos el producto a la base de datos y a nuestro archivo de productos
			products.push(newProduct);
			fs.writeFileSync(fileName, JSON.stringify(products, null, 2));

			socketServer.emit('updateProducts', products);//Esto es para el websocket

			return response.send({ status: "Success", msg: `Se ha creado un nuevo producto exitosamente con id=${newProduct.id} :)` });
		}
	} catch (error) {
		console.error("Ha surgido este error: " + error);
...
//Para actualizar un producto. Funciona con Moongose :)
...
//Vamos a eliminar un producto. Funciona con Moongose :)
...
//MOONGOSE -  Nos muestra los productos de la base de datos
